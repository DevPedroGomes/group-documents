...
async def create_signed_url(path: str, expires: int=600) -> str:
    sb = svc_client()
...
async def get_doc_owned(token: str, document_id: str):
    rows = await rest_select_with_token(token, 'documents', { 'id': 'eq.'+document_id, 'select': 'id,user_id,title,mime,storage_path' })
